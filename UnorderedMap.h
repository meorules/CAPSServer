...
public:
  ...
  int PostFunction(const string& topic, const string& message);
  const string& ListFunction();
  int CountFunction(const string& topic);
  const string& ReadFunction(const string& topic, int messagedID);
 ...
 */
inline int UnorderedMap::PostFunction(const string& topic, const string& message)
{
 ...
 */
inline int UnorderedMap::CountFunction(const string& topic)
{
  int messageCount = 0;
  if (structNotEmpty()) {
    if (TopicExists(topic)) {
 ...
 */
inline const string& UnorderedMap::ReadFunction(const string& topic, int messagedID)
{
  string message = "";
  if (structNotEmpty()) {
    if (TopicExists(topic)) {
 ...
 */
inline bool UnorderedMap::TopicExists(const string& topic) {
  {
    shared_lock<shared_mutex> mutex(lock);
