 ...
 */
inline const int topicLockMap::PostFunction(string topic, string message)
{
  int messageId;
 ...
 */
inline const string topicLockMap::ListFunction()
{
  string topicList = "";
  if (structNotEmpty()) {
    {
 ...
 */
inline const int topicLockMap::CountFunction(string topic)
{
  int messageCount = 0;
 ...
 */
inline const string topicLockMap::ReadFunction(string topic, int messagedID)
{
  string message="";
 ...
 */
inline const bool topicLockMap::TopicExists(string topic) {
  {
    if (structNotEmpty()) {
      shared_lock<shared_mutex> mutex(lock);
...
inline const bool topicLockMap::structNotEmpty() {
  {
    shared_lock<shared_mutex> mutex(lock);
