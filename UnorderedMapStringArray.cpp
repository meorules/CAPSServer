...
UnorderedMapStringArray::UnorderedMapStringArray() {
 ...
 */
void UnorderedMapStringArray::PostFunction(string topic, string message, int id)
{
  if (structNotEmpty()) {
    if (TopicExists(topic)) {
      ...
      {
        topicArray = new string[ARRAY_SIZE];
        topicArray[id] = message;
        dataStructure->at(topic) = topicArray;
      }
      else {
        if (id < ARRAY_SIZE) {
          topicArray[id] = message;
      ...
      std::unique_lock<std::shared_mutex> mutex(lock);
      string* topicArray = new string[ARRAY_SIZE];
      topicArray[id] = message;
...
string UnorderedMapStringArray::findReadTopic() {
  string toReturn = "";
  if (keyList->size() != 0) {
...
int UnorderedMapStringArray::findReadMessage(string topic) {
  int toReturn = 0;
